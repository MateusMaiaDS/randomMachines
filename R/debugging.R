...
# #
# # # Doing it for the regression settingi wou
# # subset_spam_train <- mlbench.friedman1(n = 50) %>% as.data.frame()
# # subset_spam_validation <- mlbench.friedman1(n = 50) %>% as.data.frame()
# #
# #
# # formula <- classes ~ .
# # formula <- y ~ .
...
#
# # Creating examples to be tested
# sim_data <- rmachines::sim_class(n = 10,p = 2,ratio = 0.5)
# sim_dat
# rm_mod <- rmachines::random_machines(y~.,train = sim_data,validation = sim_data,prob_model = FALSE)
# predict.rm_model_class(rm_mod,sim_data)
